...
* environments (URI template: `/environments/<end_id>`)
* workspaces (URI template: `/workspaces/<end_id>`)
* artifacts (URI template: `/artifacts/<end_id>`)
...
`POST` requests can use the `Slug` header (see https://tools.ietf.org/html/rfc5023#section-9.7[RFC5023]) to hint a preferred IRI for a resource to be created. If the IRI is not already used, it will be minted to the created resource.
...
When a resource is updated, Yggdrasil issues `POST` requests with its updated representation to all registered callbacks.  