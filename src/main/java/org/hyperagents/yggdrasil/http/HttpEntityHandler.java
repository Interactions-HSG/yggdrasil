    ...
    vertx.eventBus().request(RdfStore.BUS_ADDRESS, null, options, reply -> {
          if (reply.succeeded()) {
              ...
              if (affordance.isPresent()) {
                ...
                //TODO: Ensure mapping between the object properties and the function parameters

                else if (inputSchema.isPresent() && inputSchema.get().getDatatype() == DataSchema.OBJECT) {
                  JsonElement payload = JsonParser.parseString(entityRepresentation);
                  Map<String, Object> paramsMap = ((ObjectSchema) inputSchema.get()).parseJson(payload);
                  List<Object> params = new ArrayList<Object>(paramsMap.values());
